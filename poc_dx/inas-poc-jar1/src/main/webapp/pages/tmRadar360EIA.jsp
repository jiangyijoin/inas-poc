...
<head>
	...
	<script type="text/javascript">
		...
		function queryTmBase(){
			$.ajax({
				type : "POST",
				async : true,
				url : 'http://localhost:7081/poc/queryTmBase', // 这是数据的请求地址
				data : {
					phone : "13801605517"
				},
				dataType : "json",
		...
		function queryTmUser(){
			$.ajax({
				type : "POST",
				async : true,
				url : 'http://localhost:7080/poc/queryTmUser', // 这是数据的请求地址
				data : {
					time_id : "201703010000",
					brand : "苹果",
					model : "iPhone 6s(A1691)"
				},
				dataType : "json",
				success : function(data, textStatus, jqXHR) {
					console.log(data);
		...
		function queryTmVoiceIndex(){
			$.ajax({
				type : "POST",
				async : true,
				url : 'http://localhost:7080/poc/queryTmVoiceIndex', // 这是数据的请求地址
				data : {
					time_id : "201703010000",
					brand : "苹果",
					model : "iPhone 6s(A1691)"
		...
		function queryTmDataIndex(){
			$.ajax({
				type : "POST",
				async : true,
				url : 'http://localhost:7080/poc/queryTmDataIndex', // 这是数据的请求地址
				data : {
					time_id : "201703010000",
					brand : "苹果",
					model : "iPhone 6s(A1691)"
			...
			<div class="performance-indicator" style="overflow:hidden;">
				<div class="per-indicator-box indicator-box1 fl">
					<div class="per-indicator-box-top ">
						<span>通信用户数</span>
						<div id="tel_usrt_div"></div>个
					</div>
					
					<div class="per-indicator-box-bottom">
						<span>上网用户数</span>
						<div id="net_usrt_div"></div>个
					</div>
				</div>
				<div class="clear"></div>
			</div>
		...
		<div class="con_list" style="padding: 5px 20px 20px 20px">
			<div class="performance-indicator" style="overflow:hidden;">
				<div class="per-indicator-box indicator-box1 fl">
					<div class="per-indicator-box-top ">
						<span>4G流量分流比</span>
						<div id="lte_flow_ratio_id"></div>个
					</div>
					
					<div class="per-indicator-box-bottom">
						<span>DNS查询成功率</span>
						<div id="dns_succrate_id"></div>个
					</div>
				</div>
